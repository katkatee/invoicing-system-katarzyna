...
class AbstractControllerTest extends Specification {
    ...
    void deleteInvoice(long id) {
        mockMvc.perform(delete("$INVOICE_ENDPOINT/$id").with(csrf()))                .andExpect(status().isNoContent())
    }

    void deleteCompany(long id) {
        mockMvc.perform(delete("$COMPANY_ENDPOINT/$id").with(csrf()))                .andExpect(status().isNoContent())
    }
